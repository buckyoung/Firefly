...
FFExamples.rule184.initialize = function(FF) {
    initializeModel(FF);
    ...
    function processOff(currentCell, nextCell) {
        ...
        leftUp = leftUp ? leftUp.getState() === 'on' : false;
        up = up ? up.getState() === 'on' : false;
        rightUp = rightUp ? rightUp.getState() === 'on' : false;
    ...
    function initializeWorld(FF) {
        return function(world, width, height) {
            ...
            for (var i = 0; i < width; i++) {
                var state = (Math.random() > .40 ? 'on' : 'off');
